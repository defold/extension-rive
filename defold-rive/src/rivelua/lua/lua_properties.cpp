                                       ...
                                       ViewModelInstanceValue* propValue)
{
    switch (propValue->coreType())
    {
        ...
        case ViewModelInstanceStringBase::typeKey:
            ...
            break;
        default:
            //printf("  pushing prop: nil (unsupported type: %d)\n", propValue->coreType());
            lua_pushnil(L);
            break;
    }
}
...
ScriptedProperty::ScriptedProperty(lua_State* L,
    ...
    m_state(L), m_file(std::move(file)), m_instanceValue(std::move(value))
{
...
ScriptedPropertyViewModel::ScriptedPropertyViewModel(
    ...
    ScriptedProperty(L, std::move(file), std::move(value)), m_viewModel(std::move(viewModel))
{}
...
ScriptedViewModel::ScriptedViewModel(lua_State* L,
    ...
    m_state(L), m_file(file), m_viewModel(viewModel), m_viewModelInstance(viewModelInstance)
{}
...
{
    printf("MAWE ScriptedViewModel::pushValue: '%s', %d\n", name, coreType);

    auto itr = m_propertyRefs.find(name);
    if (itr != m_propertyRefs.end())
    {
        printf("  already a ref.\n");
    ...
    else
    {
        ...
        {
        printf("  not found. pushing nil\n");
            lua_pushnil(m_state);
        }
        else
        {
        printf("  found! pushing value!\n");
            pushViewModelInstanceValue(m_state, m_file, propValue);
        }
    }
...
int ScriptedPropertyViewModel::pushValue()
{
    ...
    else
    {
        ...
        lua_newrive<ScriptedViewModel>(m_state, m_state, m_file, m_viewModel, nullptr);
    }
...
ScriptedPropertyNumber::ScriptedPropertyNumber(
    ...
    ScriptedProperty(L, std::move(file), std::move(value))
{}
