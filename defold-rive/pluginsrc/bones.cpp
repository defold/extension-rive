...
bool ValidateBoneNames(dmArray<RiveBone*>* bones)
{
    ...
    for (uint32_t i = 0; i < num_bones; ++i)
    {
        ...
        {
            dmLogError("Bone %u has no name! Each bone must have a unique, non empty name.", i);
            status_ok = false;
        }
        ...
        for (uint32_t j = i+1; j < num_bones; ++j)
        {
            ...
            {
                dmLogError("Two bones have the name '%s'! Each bone must have a unique, non empty name.", cur_name);
                status_ok = false;
            }
        }
    }
